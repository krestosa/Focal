...
GLCLI-001 defines the stable command surface. GLCLI-002 adds a real EGL
surfaceless/pbuffer context probe on platforms that expose libEGL and libGL.
Later roadmap units implement compile, render and suite execution.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="focal-gl", description="Standalone OpenGL validation harness for Focal shaders.")
    parser.add_argument("--version", action="version", version=f"%(prog)s {VERSION}")
    subparsers = parser.add_subparsers(dest="command", required=True)
    for command, description in (("probe", "Create a context and report capabilities."), ("compile", "Compile and link a selected shader program."), ("render", "Render a deterministic fixture and read it back."), ("suite", "Run a declared fixture matrix.")):
...
def _egl_probe(requested_version: str, requested_profile: str, size: str) -> dict[str, Any]:
    ...
    EGL_RED_SIZE, EGL_GREEN_SIZE, EGL_BLUE_SIZE, EGL_ALPHA_SIZE, EGL_DEPTH_SIZE = 0x3024, 0x3023, 0x3022, 0x3021, 0x3025
    EGL_NONE, EGL_WIDTH, EGL_HEIGHT = 0x3038, 0x3057, 0x3056
    EGL_CONTEXT_MAJOR_VERSION, EGL_CONTEXT_MINOR_VERSION, EGL_CONTEXT_OPENGL_PROFILE_MASK = 0x3098, 0x30FB, 0x30FD
    EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT, EGL_CONTEXT_OPENGL_COMPATIBILITY_PROFILE_BIT = 1, 2
    ...
    if platform_ptr:
        get_platform_display = ctypes.CFUNCTYPE(EGLDisplay, ctypes.c_uint, ctypes.c_void_p, ctypes.POINTER(EGLint))(platform_ptr)
    ...
    try:
        ...
        config_attributes = (EGLint * 15)(EGL_SURFACE_TYPE, EGL_PBUFFER_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_BIT, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_DEPTH_SIZE, 24, EGL_NONE)
        config = EGLConfig()
        config_count = EGLint()
        if not egl.eglChooseConfig(display, config_attributes, ctypes.byref(config), 1, ctypes.byref(config_count)) or config_count.value < 1:
        ...
        profile_bit = EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT if requested_profile == "core" else EGL_CONTEXT_OPENGL_COMPATIBILITY_PROFILE_BIT
        context_attributes = (EGLint * 7)(EGL_CONTEXT_MAJOR_VERSION, req_major, EGL_CONTEXT_MINOR_VERSION, req_minor, EGL_CONTEXT_OPENGL_PROFILE_MASK, profile_bit, EGL_NONE)
        context = egl.eglCreateContext(display, config, EGL_NO_CONTEXT, context_attributes)
        if not context:
            raise ContextUnavailable(f"eglCreateContext {requested_version} {requested_profile} failed: 0x{egl.eglGetError():04x}")
        ...
        GL_VENDOR, GL_RENDERER, GL_VERSION, GL_EXTENSIONS, GL_SHADING_LANGUAGE_VERSION = 0x1F00, 0x1F01, 0x1F02, 0x1F03, 0x8B8C
        GL_MAX_COLOR_ATTACHMENTS, GL_MAX_DRAW_BUFFERS, GL_MAX_TEXTURE_SIZE = 0x8CDF, 0x8824, 0x0D33
        strings = {"vendor": _decode_gl_string(gl.glGetString(GL_VENDOR)), "renderer": _decode_gl_string(gl.glGetString(GL_RENDERER)), "version": _decode_gl_string(gl.glGetString(GL_VERSION)), "glslVersion": _decode_gl_string(gl.glGetString(GL_SHADING_LANGUAGE_VERSION))}
        limits: dict[str, int] = {}
        for name, enum in (("maxColorAttachments", GL_MAX_COLOR_ATTACHMENTS), ("maxDrawBuffers", GL_MAX_DRAW_BUFFERS), ("maxTextureSize", GL_MAX_TEXTURE_SIZE)):
            value = ctypes.c_int(); gl.glGetIntegerv(enum, ctypes.byref(value)); limits[name] = value.value
        extension_names = sorted(set((_decode_gl_string(gl.glGetString(GL_EXTENSIONS)) or "").split()))
        return {"backend": backend, "eglVersion": f"{major.value}.{minor.value}", "requestedVersion": requested_version, "requestedProfile": requested_profile, **strings, "limits": limits, "extensions": extension_names, "capabilities": {"framebuffer": limits["maxColorAttachments"] > 0, "geometry": any("geometry_shader" in ext.lower() for ext in extension_names), "tessellation": any("tessellation_shader" in ext.lower() for ext in extension_names), "compute": any("compute_shader" in ext.lower() for ext in extension_names), "images": any("shader_image_load_store" in ext.lower() for ext in extension_names), "ssbo": any("shader_storage_buffer_object" in ext.lower() for ext in extension_names), "debugOutput": any("debug" in ext.lower() for ext in extension_names), "timerQuery": any("timer_query" in ext.lower() for ext in extension_names)}}
    finally:
        if display:
            egl.eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)
            if surface: egl.eglDestroySurface(display, surface)
            if context: egl.eglDestroyContext(display, context)
            egl.eglTerminate(display)


def _probe_result(args: argparse.Namespace) -> Result:
    if args.backend not in ("auto", "egl"):
        return Result(SCHEMA_VERSION, VERSION, args.command, "UNSUPPORTED", EXIT_CONTEXT_UNAVAILABLE, "STATIC", f"backend {args.backend} is not implemented by GLCLI-002", str(args.artifacts) if args.artifacts else None)
    try:
        context = _egl_probe(args.gl_version, args.gl_profile, args.size)
    except (ContextUnavailable, OSError) as exc:
        return Result(SCHEMA_VERSION, VERSION, args.command, "UNSUPPORTED", EXIT_CONTEXT_UNAVAILABLE, "STATIC", str(exc), str(args.artifacts) if args.artifacts else None)
    return Result(SCHEMA_VERSION, VERSION, args.command, "PASS", EXIT_OK, "STATIC", "real OpenGL context created and queried; shader compile/link evidence remains pending", str(args.artifacts) if args.artifacts else None, context)


def _not_implemented_result(args: argparse.Namespace) -> Result:
    return Result(SCHEMA_VERSION, VERSION, args.command, "UNSUPPORTED", EXIT_CONTEXT_UNAVAILABLE, "STATIC", "the command requires GLCLI-003 or a later runtime unit", str(args.artifacts) if args.artifacts else None)


def emit(result: Result, json_output: bool) -> None:
    if json_output:
        print(json.dumps(asdict(result), sort_keys=True, separators=(",", ":"))); return
    print(f"focal-gl {result.harnessVersion}: {result.command}: {result.outcome}")
    print(result.message)
    if result.context: print(f"backend={result.context['backend']} renderer={result.context.get('renderer')} version={result.context.get('version')}")
