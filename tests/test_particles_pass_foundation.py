...
VERTEX = ROOT / "shaders" / "gbuffers_particles.vsh"
FRAGMENT = ROOT / "shaders" / "gbuffers_particles.fsh"


def test_particles_program_stages_exist() -> None:
    assert VERTEX.is_file()
    assert FRAGMENT.is_file()


def test_particles_vertex_stage_has_expected_interfaces() -> None:
...
def test_particles_fragment_stage_is_bounded_and_lit() -> None:
    ...
    assert "gl_FragData[0]" in source
    assert "for (" not in source
    assert "while (" not in source
