...
VERTEX = ROOT / "shaders" / "gbuffers_terrain_translucent.vsh"
FRAGMENT = ROOT / "shaders" / "gbuffers_terrain_translucent.fsh"


def test_terrain_translucent_program_stages_exist() -> None:
    assert VERTEX.is_file()
    assert FRAGMENT.is_file()


def test_terrain_translucent_vertex_stage_has_expected_interfaces() -> None:
...
def test_terrain_translucent_fragment_stage_preserves_alpha() -> None:
    ...
    assert "if (albedo.a < 0.01)" in source
