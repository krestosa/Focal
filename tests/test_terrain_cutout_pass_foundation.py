...
VERTEX = ROOT / "shaders" / "gbuffers_terrain_cutout.vsh"
FRAGMENT = ROOT / "shaders" / "gbuffers_terrain_cutout.fsh"


def test_terrain_cutout_program_stages_exist() -> None:
    assert VERTEX.is_file()
    assert FRAGMENT.is_file()


def test_terrain_cutout_vertex_stage_has_expected_interfaces() -> None:
...
def test_terrain_cutout_fragment_stage_is_bounded_and_lit() -> None:
    ...
    assert "if (albedo.a < 0.5)" in source
