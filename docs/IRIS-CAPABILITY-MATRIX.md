...
- Focal baseline: `d66834fc9cff7375da50930c3572eaaa55cdd008`
...
| `IRIS-GL-005` | Terminal OpenGL compile/link/render harness | PARCIAL | The stable `focal-gl` command contract is merged through PR #62. PR #71 merged a real EGL surfaceless/default-display pbuffer context probe with Validation run `30480734608` and merge `d466d6bca8d4e6ad2bfc40d94bc0338c54bf0895`. PR #74 added an explicit Ubuntu 24.04 Mesa llvmpipe EGL gate at feature head `bbe2ed5883a298f16853eb2477dbde0e82909296`; Validation run `30490039060` succeeded and merge `70d0e31f76cd4ede71876b44e5f1966788d81d0a` is on `main`. PR #75 published the canonical Mesa evidence at merge `f8390b650455eebfb74ad332d8a9b91eff9f2495`. PR #77 added robust core-profile extension enumeration through `GL_NUM_EXTENSIONS` and `glGetStringi`, retained a controlled legacy fallback, passed Validation run `30493688616`, and merged as `d66834fc9cff7375da50930c3572eaaa55cdd008`. The probes report a real EGL backend, vendor, renderer, GL/GLSL versions, limits and an explicit extension-enumeration API. Canonical evidence remains `STATIC` because no shader stage was compiled or linked. | Preserve the real EGL and Mesa llvmpipe routes, indexed extension enumeration and factual `UNSUPPORTED` behavior. Do not claim `GL_COMPILE_LINK`, render/readback, patched-source or client acceptance. Hidden-window/platform routes remain required; hardware evidence remains separate from software Mesa. | Finish the remaining `GLCLI-002` backend gates; then compile/link gbuffers/composite/final fixtures, framebuffer render/readback, GL error and NaN/Inf checks, determinism, patched output and locked client. | `QA-003`, `GLCLI-001`–`GLCLI-008`, `INT-001` |
...
Merged `GLCLI-002` evidence establishes that the implementation can create and interrogate a real EGL context on the supported project route and on an explicit Mesa llvmpipe CI route. PR #77 additionally establishes robust indexed extension enumeration for core profiles with a controlled compatibility fallback. This does not raise the canonical evidence level above `STATIC`, because no shader stage or complete program was compiled and linked.
...
`IRIS-001`, `IRIS-002`, `IRIS-003`, `IRIS-004` and the `GLCLI-001` command contract are complete at their current `STATIC` evidence level. `GLCLI-002` is `EN PROGRESO`: the real EGL probe, explicit Mesa llvmpipe CI probe and robust indexed core-profile extension enumeration are merged, while controlled hidden-window/platform routes remain pending. No shader compile/link or render/readback acceptance has been claimed.

Next prioritized unit: finish the remaining `GLCLI-002` hidden-window/platform backend gates, then implement the minimum `GLCLI-004/005` compile-link-render-readback path. `IRIS-005` remains the next Iris-format contract and may proceed only when it does not delay the harness foundation.