...
- Focal baseline: `cc85e7c4f221ad7f4298f5a96ea34f3c227ac314`
...
10. execute in an isolated worker with watchdog — pending;
...
`IRIS-001`, `IRIS-002`, `IRIS-003`, `IRIS-004`, `GLCLI-001`, `GLCLI-003`, `GLCLI-004` and `GLCLI-005` are complete at their declared evidence boundaries. `GLCLI-002` remains `EN PROGRESO`: real EGL and controlled hidden GLFW context/probe evidence on Mesa llvmpipe plus robust indexed core-profile extension enumeration are merged, while native WGL/CGL routes and representative physical-GPU evidence remain unaccepted. Standalone framebuffer readback is accepted only for the exact PR #97 Mesa fixtures; Iris-patched and client evidence remain pending.

Next prioritized unit: implement `GLCLI-007` isolated worker execution, watchdog timeout/crash/context-loss classification, partial-artifact preservation and cleanup. Continue native `GLCLI-002` routes in parallel only when they do not delay that safety boundary. `IRIS-005` remains the next Iris-format contract and may proceed only when it does not delay the harness foundation.
