...
- Focal baseline: `f8390b650455eebfb74ad332d8a9b91eff9f2495`
...
| `IRIS-GL-005` | Terminal OpenGL compile/link/render harness | PARCIAL | The stable `focal-gl` command contract is merged through PR #62. PR #71 merged a real EGL surfaceless/default-display pbuffer context probe with Validation run `30480734608` and merge `d466d6bca8d4e6ad2bfc40d94bc0338c54bf0895`. PR #74 added an explicit Ubuntu 24.04 Mesa llvmpipe EGL gate at feature head `bbe2ed5883a298f16853eb2477dbde0e82909296`; Validation run `30490039060` succeeded and merge `70d0e31f76cd4ede71876b44e5f1966788d81d0a` is on `main`. PR #75 published the canonical Mesa evidence at merge `f8390b650455eebfb74ad332d8a9b91eff9f2495`. The probes report a real EGL backend, vendor, renderer, GL/GLSL versions and limits. Canonical evidence remains `STATIC` because no shader stage was compiled or linked. | Preserve the real EGL and Mesa llvmpipe routes and factual `UNSUPPORTED` behavior. Do not claim `GL_COMPILE_LINK`, render/readback, patched-source or client acceptance. Hidden-window/platform routes and robust core-profile extension enumeration remain required; hardware evidence remains separate from software Mesa. | Finish the remaining `GLCLI-002` backend and extension gates; then compile/link gbuffers/composite/final fixtures, framebuffer render/readback, GL error and NaN/Inf checks, determinism, patched output and locked client. | `QA-003`, `GLCLI-001`–`GLCLI-008`, `INT-001` |
...
`IRIS-001`, `IRIS-002`, `IRIS-003`, `IRIS-004` and the `GLCLI-001` command contract are complete at their current `STATIC` evidence level. `GLCLI-002` is `EN PROGRESO`: the real EGL probe and explicit Mesa llvmpipe CI probe are merged and documented, while controlled hidden-window/platform routes and robust core-profile extension enumeration remain pending. No shader compile/link or render/readback acceptance has been claimed.

Next prioritized unit: finish the remaining `GLCLI-002` backend and extension gates, then implement the minimum `GLCLI-004/005` compile-link-render-readback path. `IRIS-005` remains the next Iris-format contract and may proceed only when it does not delay the harness foundation.