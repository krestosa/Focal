...
- Baseline audited: `66652dec9471b23f34bd3ad63aeb1d07716ec203`
- Roadmap schema revision: `17`
...
| OpenGL/GLSL | 🟡 EN PROGRESO | EGL and hidden GLFW create real Mesa llvmpipe contexts; PR #93 adds real stage compilation and program link for representative gbuffers, composite and final-equivalent fixtures. Native WGL, CGL/NSOpenGL, framebuffer readback and representative physical-GPU evidence remain pending. | [Programs][iris-programs], [OpenGL extensions][iris-extensions], [macOS limits][iris-macos] |
...
| [x] 🟢 `IRIS-004` | P0 | Output constants and directives: `RENDERTARGETS`, `DRAWBUFFERS`, formats, clears, clear colors and blend. | `spec/iris-output-directives.json` and `tests/test_iris_output_directives.py` are merged through PR #58 with Validation run `30421966410`; runtime attachment acceptance remains assigned to `GLCLI-005`. | [Constants][iris-constants], [RENDERTARGETS][iris-rendertargets], [DRAWBUFFERS][iris-drawbuffers], [Buffer format][iris-buffer-format] | Preserve the static contract and add runtime framebuffer evidence. |
...
1. Finish native `GLCLI-002` WGL and CGL/NSOpenGL routes and representative physical-GPU evidence while completing `GLCLI-005` framebuffer draw/readback; do not begin visual shader features before that minimum runtime path is established.
...
`GLCLI-005 minimum render path`: create a deterministic framebuffer, draw the accepted gbuffers/composite/final sequence and read color/depth back with finite-value and range invariants, without claiming Iris-client equivalence.
