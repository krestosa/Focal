...
- Audit UTC: `2026-07-31`
- Baseline audited: `d46a0649a67e8853a3646c6117d32ae63c2bd61a`
- Roadmap schema revision: `21`
...
- Documentation rule: every feature row contains at least one direct official Iris documentation link, reviewed on `2026-07-31` UTC.
...
| OpenGL/GLSL | 🟡 EN PROGRESO | EGL and hidden GLFW create real Mesa llvmpipe contexts; PR #93 adds real stage compilation/program link, PR #97 adds deterministic framebuffer draw plus finite color/depth readback for representative gbuffers, composite and final-equivalent fixtures, PR #99 adds an isolated worker supervisor, PR #103 publishes a deterministic Mesa evidence manifest and retained artifact bundle, and PR #105 adds the native hidden WGL adapter with canonical evidence in PR #106. WGL remains bounded `STATIC` implementation evidence until a native Windows driver execution is recorded. CGL/NSOpenGL, representative physical-GPU evidence and comprehensive driver context-loss detection remain pending. | [Programs][iris-programs], [OpenGL extensions][iris-extensions], [macOS limits][iris-macos] |
...
| [ ] 🟡 `GLCLI-002` | P0 | Real offscreen context creation and capability probe across EGL, hidden-window and native platform backends. | PR #71 merged the real EGL pbuffer probe with Validation run `30480734608` and evidence in [`docs/evidence/GLCLI-002-EGL-PROBE.md`](evidence/GLCLI-002-EGL-PROBE.md). PR #74 added the explicit Mesa llvmpipe EGL gate; Validation run `30490039060` succeeded, merge `70d0e31f76cd4ede71876b44e5f1966788d81d0a` is on `main`, and PR #75 published [`docs/evidence/GLCLI-002-MESA-CI-PROBE.md`](evidence/GLCLI-002-MESA-CI-PROBE.md). PR #77 added `GL_NUM_EXTENSIONS` plus `glGetStringi` enumeration with a controlled legacy fallback; Validation run `30493688616` succeeded and merge `d66834fc9cff7375da50930c3572eaaa55cdd008` is on `main`. PR #79 merged the controlled hidden-window GLFW backend. PR #80 published [`docs/evidence/GLCLI-002-GLFW-CI-PROBE.md`](evidence/GLCLI-002-GLFW-CI-PROBE.md), recording validated head `12dc8e24f9339193743f2372b2b8e785ebc60aa7` and successful Validation run `30500297439`. The GLFW fixture created a real hidden context on Mesa llvmpipe under Xvfb and reported capabilities. PR #105 added a native hidden WGL adapter, explicit isolated-worker routing and bounded context-failure mapping; Validation run `30601640571` succeeded on exact head `1259023ef67597b25894544934508b5ef27ba96c`, merge `1234c4c55f8990a92121065e3b85596374b162c6` is on `main`, and PR #106 published [`docs/evidence/GLCLI-002-WGL-PROBE.md`](evidence/GLCLI-002-WGL-PROBE.md). The accepted WGL evidence is `STATIC` implementation and cross-platform routing evidence, not successful execution against a native Windows driver. GLCLI-002 remains context/probe evidence only; `GLCLI-004` covers representative compilation/link and `GLCLI-005` covers representative framebuffer draw/readback. | [OpenGL extensions][iris-extensions], [macOS limits][iris-macos] | Execute the WGL route on representative native Windows hardware, implement and exercise CGL/NSOpenGL where supported, and collect representative physical-GPU evidence. |
...
1. Preserve the completed `GLCLI-007` isolation boundary and the native WGL adapter while collecting a representative native Windows execution, implementing CGL/NSOpenGL where supported and gathering representative physical-GPU evidence; do not bypass the worker supervisor for risky visual shader or multipass work.
...
- `2026-07-31` — Revision 21 reconciled the native hidden WGL adapter from PR #105, exact-head Validation run `30601640571`, merge `1234c4c55f8990a92121065e3b85596374b162c6` and canonical evidence PR #106. WGL advances to bounded `STATIC` implementation evidence; native Windows driver execution, CGL/NSOpenGL and representative physical-GPU records remain pending.

## Next prioritized unit

`GLCLI-002 native Windows execution, CGL/NSOpenGL and physical-GPU evidence`: execute the merged WGL route on representative Windows hardware, implement and exercise CGL/NSOpenGL where supported, and collect representative physical-GPU records under the GLCLI-008 procedure; preserve the completed worker, WGL adapter and evidence-manifest boundaries.
